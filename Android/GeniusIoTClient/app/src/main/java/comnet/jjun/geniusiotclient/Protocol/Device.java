...
public class Device {
    ...
    public Device(int id, int type, String name, String address, int value1, int value2, BluetoothLeManager ble){
        this.Device_id = id;
...
//        Log.d("Device" , "Num of properties : " + deviceString.length);
    ...
    public void setBle(BluetoothLeManager ble){
        this.ble = ble;
    }
    ...
    public static HashMap<Integer, Integer> getBathInformation(int value1, int value2) {
        ...
        bath.put(GeniusProtocol.BATH_EXECUTION_REQUEST, execution_request);
        int water_request = ((value1 ) & 0b01100000) >> 5;
        bath.put(GeniusProtocol.BATH_WATER_REQUEST, water_request);
        int temp_request = (value1 ) & 0b00011111;
        bath.put(GeniusProtocol.BATH_TEMP_REQUEST, temp_request);

        // 1.result
        int execution_result = ((value2 ) & 0b10000000) >> 7;
        bath.put(GeniusProtocol.BATH_EXECUTION_RESULT, execution_result);
        int water_result = ((value2) & 0b01100000) >> 5;
        bath.put(GeniusProtocol.BATH_WATER_RESULT, water_result);
        int temp_result = (value2 ) & 0b00011111;
        bath.put(GeniusProtocol.BATH_TEMP_RESULT, temp_result);
    ...
    public static int[] setBathInformation(HashMap<Integer,Integer> bath){
        ...
        //request
        int execution_request = bath.get(GeniusProtocol.BATH_EXECUTION_REQUEST);
        int water_request = bath.get(GeniusProtocol.BATH_WATER_REQUEST);
        int temp_request = bath.get(GeniusProtocol.BATH_TEMP_REQUEST);
        int execution_result = bath.get(GeniusProtocol.BATH_EXECUTION_RESULT);
        int water_result = bath.get(GeniusProtocol.BATH_WATER_RESULT);
        int temp_result = bath.get(GeniusProtocol.BATH_TEMP_RESULT);
