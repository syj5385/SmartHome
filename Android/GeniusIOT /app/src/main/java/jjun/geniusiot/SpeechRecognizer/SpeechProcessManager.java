...
public class SpeechProcessManager {
    ...
    public static final int REQUEST_CURRENT_TEMPERATURE=22;
    ...
    public SpeechProcessManager(Context context, String speech, Handler mHandler) {
        ...
        if((selected_command = processRequestCommand(speech)) == -1){
            ...
            if(selected_command == REQUEST_SCAN_DEVICE){
                ...
                deviceDB = new DeviceDB(context,DeviceDB.DeviceTable);
...
//        String[] speech_arr = speech.split("");
    ...
    private int processRequestDevice(String speech) {
        ...
        for(int i=0; i<d_list.size(); i++){
            String device_name = d_list.get(i).getDevice_name().replaceAll(" ","");

            if (speech.contains(device_name)) {
    ...
    public static String getTTS_String(Device device,int command){
        ...
        switch(command){
            ...
            case REQUEST_WINDOW_MORE_CLOSE:
                if(device.getDeviceType() == Command.WINDOW) {
                    tts = "를 조금 닫겠습니다.";
                }
                else{
            ...
            case REQUEST_CURRENT_TEMPERATURE :
                if(device.getDeviceType() == Command.TEMP){
                    tts = "를 알려드리겠습니다. 현재 실내 온도는 섭씨" + String.valueOf(device.getvalue()[0]) + "도이며 실내 습도는 "+ String.valueOf(device.getvalue()[1]) +"퍼센트 입니다.";
                }
                else{
                    tts = "계가 아니어서 온도를 알려드릴 수 없습니다.";
                }
                break;
